...
export default function shipCountAlarmsEpic(
  action: Observable<Action>,
  state: StateObservable<Store>,
) {
  return action.pipe(
    ofType(
      ActionType.CheckShip,
    ...
    concatMap((action) => {
      ...
      const babies = activePlayers.filter((player) =>
        !player.info.highestRoleName
      ).length
      if (
        babies >= (shipType.babySpots || DefaultBabySpots) &&
        !(ship.alarms && ship.alarms.babies)
      ) {
      ...
      if (
        activePlayers.length <= lowSpots && !(ship.alarms && ship.alarms.low)
      ) {
        alarms.push(
          alarmShipLow(action.fleet, action.ship, activePlayers.length),
        )
      }
      const veryLowSpots = shipType.veryLowSpots || DefaultVeryLowSpots
      if (activePlayers.length <= veryLowSpots) {
        alarms.push(
          alarmShipVeryLow(action.fleet, action.ship, activePlayers.length),
        )
      }
