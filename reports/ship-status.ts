...
export default function shipStatus(ship: Ship, players: PlayerStore) {
  ...
  const shipBabyAlarm = alarms.babies
    ? ` 👶${alarms.babies}/${shipType.total}`
    : ''
  const shipLowAlarm = alarms.low || alarms.low === 0
    ? `${
      alarms.low <= shipType.veryLowSpots ? ' 🚨👤' : ' 👤'
    }${alarms.low}/${shipType.total}`
    : ''
...
${activePlayerStatus}${visitingPlayers ? ` ⤴${visitingPlayers}` : ''}${
